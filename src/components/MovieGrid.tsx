...
export default function MovieGrid({ movies, onMovieClick }: MovieGridProps) {
    ...
    if (viewMode === 'list') {
        ...
        <div className="space-y-4">
            {movies.map((movie) => (
            <div key={movie.id} className="flex gap-4 p-4 border rounded-lg hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => onMovieClick?.(movie)}>
                <img
                src={movie.poster}
                alt={movie.title}
                ...
                <div className="flex items-center gap-4 mt-3 text-sm text-muted-foreground">
                    ...
                    {movie.viewCount && <span>{movie.viewCount.toLocaleString()} views</span>}
                </div>
                </div>
            </div>
            ))}
